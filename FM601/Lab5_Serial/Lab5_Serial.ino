...
void coaster(int base, int speedBase) {
  ...
  while (true) {
    ...
    static unsigned long serialTimer = millis();  // 宣告靜態的計時器變數
    if(millis() - serialTimer > 1000) {   // 每秒顯示一次
      serialTimer = millis();             // 重設計時器
      Serial.println(String("vin=") +vin+ " faster=" +faster+ " red=" +(int)red+ " green=" +(int)green+
                    " (base=" +base+ " speedBase=" +speedBase+ ")");
    }

    // 使用自訂的myMillis(速度)來計算呼吸燈的亮度, 
    //     再對映到紅、綠的亮度, 藍的亮度固定為 0
...
unsigned long myMillis(float faster) {
  static unsigned long lastMs = millis();  // 儲存最近一次讀取的 millis() 值
